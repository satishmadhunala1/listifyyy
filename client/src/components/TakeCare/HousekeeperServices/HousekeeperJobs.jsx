...
export default function HousekeeperJobs() {
  const [activeTab, setActiveTab] = useState('housekeepers');

  const housekeeperJobs = [
    {
      title: "Regular Home Cleaning",
      zipCode: "11801",
      location: "Hicksville, NY",
      payrate: "$20-30",
      workType: "Weekly & Bi-weekly",
      languages: "Hindi, English, Telugu",
      services: "House Cleaning, Laundry, Organ.."
    },
    {
      ...
      location: "Yonkers, NY",
      payrate: "$25-40",
      workType: "One-time Service",
      languages: "English,",
      services: "Deep Cleaning, Kitchen, Bath.."
    },
    {
      ...
      services: "Full House Cleaning, Windows.."
    }
  ];
  ...
  const housekeepers = [
    ...
    },
    {
      ...
      specialties: "Eco-friendly Cleaning, Laundry"
    }
  ];
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="text-center mb-6 sm:mb-8 lg:mb-10 px-2">
          <div className="flex items-center justify-center gap-2 mb-3 sm:mb-4">
            <span className="text-[#27BB97] text-xs xs:text-sm font-medium">Housekeeping Services</span>
            <Sparkles className="w-4 h-4 xs:w-5 xs:h-5 text-[#27BB97]" />
          </div>
          <h1 className="text-2xl xs:text-3xl sm:text-4xl lg:text-5xl font-bold text-gray-900 mb-4 sm:mb-5 lg:mb-6 leading-tight">
            Housekeepers and Housekeeping Jobs In New York,NY
          </h1>
          ...
          <div className="inline-flex bg-white rounded-full p-1 shadow-sm sm:shadow-md border border-gray-200 mb-4 sm:mb-6">
            ...
            <button
              ...
              aria-label="View housekeeping jobs"
            >
              Housekeeping Jobs
            </button>
          </div>
        ...
        {activeTab === 'jobs' ? (
          <>
            ...
            <div className="text-center">
              <button 
                ...
                aria-label="View all housekeeping jobs"
              >
                View Housekeeping Jobs
              </button>
            </div>
          </>
        ) : (
          <>
              ...
              {housekeepers.map((housekeeper, index) => (
                <div key={index} className="bg-white rounded-xl shadow-sm hover:shadow-md sm:shadow-md border border-gray-200 overflow-hidden transition-all duration-300 hover:scale-[1.02]">
                  <div className="p-4 xs:p-5 sm:p-6">
                    ...
                    <div className="space-y-1.5 xs:space-y-2 mb-3 xs:mb-4">
                      <p className="text-xs xs:text-sm text-gray-600">
                        ZIP Code: {housekeeper.zipCode}, <span className="text-cyan-500">{housekeeper.location}</span>
                      </p>
                      <p className="text-xs xs:text-sm text-gray-600">
                        Hourly Rate: <span className="text-green-500 font-semibold">{housekeeper.rate}</span> <span className="text-gray-400">/Hour</span>
                      </p>
                    </div>
                    ...
                    <div className="border-t border-gray-200 pt-3 xs:pt-4 space-y-2 xs:space-y-3">
                      <div className="flex">
                        ...
                        <span className="text-xs xs:text-sm text-gray-900">{housekeeper.experience}</span>
                      </div>
                      <div className="flex">
                        <span className="text-xs xs:text-sm text-gray-500 w-24 xs:w-32">Languages</span>
                        <span className="text-xs xs:text-sm text-gray-900 truncate">{housekeeper.languages}</span>
                      </div>
                      <div className="flex">
                        <span className="text-xs xs:text-sm text-gray-500 w-24 xs:w-32">Specialties</span>
                        <span className="text-xs xs:text-sm text-gray-900 truncate">{housekeeper.specialties}</span>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="px-4 xs:px-5 sm:px-6 pb-4 xs:pb-5 sm:pb-6">
                    <button 
                      className="w-full bg-white border border-gray-300 text-gray-700 py-2 xs:py-2.5 sm:py-3 px-4 rounded-full hover:bg-gray-50 transition-colors duration-200 flex items-center justify-center gap-2 text-xs xs:text-sm focus:outline-none focus:ring-2 focus:ring-[#27BB97]"
                      aria-label={`View profile for ${housekeeper.name}`}
                    >
                      View profile
                      <span className="text-lg">→</span>
                    </button>
            ...
            {/* View More Housekeepers Button (Only for housekeepers tab) */}
            <div className="text-center">
              <button 
                className="bg-white border border-gray-300 text-gray-700 py-2.5 xs:py-3 sm:py-3 px-6 xs:px-8 sm:px-10 rounded-full hover:bg-gray-50 transition-colors duration-200 shadow-sm text-xs xs:text-sm sm:text-base focus:outline-none focus:ring-2 focus:ring-[#27BB97]"
                aria-label="View all housekeepers"
              >
                View More Housekeepers
              </button>
            </div>
          </>
        )}
